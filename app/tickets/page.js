...
export default function AdminTickets() {
  ...
  const handleUpdateTicketStatus = async (ticketId, newStatus) => {
    try {
      const requests = await supportRequestsAPI.updateRequest(
        ticketId,
        {status: newStatus}
      );
      console.log(requests);

      dispatch(updateSupportRequest({ _id: ticketId, status: newStatus }));
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <div className="flex items-center text-sm text-gray-500">
                        <p>
                          {ticket?.userId.name} ({ticket?.userType})
                        </p>
                        <p className="ml-2">|| {ticket?.device}</p>
                      </div>
                    </div>
                  </div>
